...
object BrowseTree {
    ...
    fun childrenOf(library: Library, parentId: String): List<BrowseNode> = when {
        parentId == BrowseIds.ROOT -> rootChildren()
        ...
        else -> songsOf(library, parentId).map { it.toNode(parentId) }
    }

    /** The tracks a parent stands for, which is also the queue built when one is picked. */
    fun songsOf(library: Library, parentId: String): List<Song> = when {
    ...
    fun search(library: Library, query: String): List<BrowseNode> {
        val results = LibraryIndex.search(library, query)
        val fromAlbums = results.albums.map { album ->
            BrowseNode(
                id = BrowseIds.album(album.id),
                title = album.title,
                subtitle = album.artist,
